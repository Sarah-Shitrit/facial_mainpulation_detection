...
class FacesDataset(Dataset):
    ...
    def __getitem__(self, index) -> tuple([torch.Tensor, int]):
        ...
        if index < len(self.real_image_names):
            im = PIL.Image.open(os.path.join(self.root_path, 'real', self.real_image_names[index]))
            label = 0
        else:
            im = PIL.Image.open(os.path.join(self.root_path, 'fake', self.fake_image_names[index - len(self.real_image_names)]))
            label = 1
