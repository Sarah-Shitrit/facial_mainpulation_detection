...
def get_soft_scores_and_true_labels(dataset, model):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(train_dataloader):
            ...
            final_output.append(predictions)
            label_output.append(targets)

    all_soft_scores = torch.cat(final_output, dim=0)
    all_label = torch.cat(label_output, dim=0)
    all_first_soft_scores, all_second_soft_scores = all_soft_scores[:,0], all_soft_scores[:,1]

    return all_first_soft_scores, all_second_soft_scores, all_label
    # instead of return torch.rand(100, ), torch.rand(100, ), torch.randint(0, 2, (100, ))
