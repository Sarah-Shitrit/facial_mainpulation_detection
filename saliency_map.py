...
def compute_gradient_saliency_maps(samples: torch.tensor,
    ...
    corresponded_scores = scores.gather(1, true_labels.view(-1, 1)).squeeze()
    corresponded_scores.backward(torch.ones_like(true_labels))
    # gradients from the samples object
    grad_from_samples = samples.grad.data
    abs_grad_from_samples = torch.abs(grad_from_samples)
    saliency, _ = abs_grad_from_samples.max(dim=1)  # take max value

    return saliency  # instead of return torch.rand(6, 256, 256)
