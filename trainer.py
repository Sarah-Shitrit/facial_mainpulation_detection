...
class Trainer:
    ...
    def train_one_epoch(self) -> tuple([float, float]):
        ...
        for batch_idx, (inputs, targets) in enumerate(train_dataloader):
            ...
            loss = self.criterion(pred, targets) #pred:ind[0]-realness, ind[1]-fakeness, targets:0-real, 1-fake
            ...
            correct_labeled_samples += sum(pred.argmax(dim=1) == targets).item()

            avg_loss = total_loss / nof_samples
            accuracy = correct_labeled_samples / nof_samples
            """end of our addition to code"""
