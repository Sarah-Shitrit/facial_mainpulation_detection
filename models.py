...
class SimpleNet_lessParams(nn.Module):
    """Simple Convolutional and Fully Connect network."""
    def __init__(self):
        super().__init__()
        self.conv1 = nn.Conv2d(3, 6, kernel_size=(7, 7))
        self.pool = nn.AvgPool2d(2, 2)
        self.conv2 = nn.Conv2d(6, 12, kernel_size=(7, 7))
        self.conv3 = nn.Conv2d(12, 14, kernel_size=(7, 7))
        self.fc1 = nn.Linear(14 * 26 * 26, 1024)
        self.fc2 = nn.Linear(1024, 256)
        self.fc3 = nn.Linear(256, 2)

    def forward(self, image):
        """Compute a forward pass."""
        first_conv_features = self.pool(F.leaky_relu(self.conv1(image)))
        second_conv_features = self.pool(F.leaky_relu(self.conv2(
            first_conv_features)))
        third_conv_features = self.pool(F.leaky_relu(self.conv3(
            second_conv_features)))
        # flatten all dimensions except batch
        flattened_features = torch.flatten(third_conv_features, 1)
        fully_connected_first_out = F.leaky_relu(self.fc1(flattened_features))
        fully_connected_second_out = F.leaky_relu(self.fc2(fully_connected_first_out))
        two_way_output = self.fc3(fully_connected_second_out)
        return two_way_output
...
def get_xception_based_model() -> nn.Module:
    ...
    classification = nn.Sequential(nn.Linear(2048, 1000),
    ...
    custom_network.fc = classification
    return custom_network  # instead of return SimpleNet()
